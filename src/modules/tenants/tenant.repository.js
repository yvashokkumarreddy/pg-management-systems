...
export async function findRoomById(dbClient, roomId) {
...
export async function createTenant(dbClient, data) {
  const result = await dbClient
    .insert(tenants)
    .values(data)
    .returning();
...
export async function findTenantDetailsById(
  ...
  const roomResult = tenant.roomId
    ? await dbClient
        .select()
        .from(rooms)
  ...
  const depositResult = await dbClient
    .select()
    .from(tenantDeposits)
    .where(eq(tenantDeposits.tenantId, tenantId))
    .limit(1);

  const bills = await dbClient
    .select()
    .from(rentBills)
    .where(eq(rentBills.tenantId, tenantId))
    .orderBy(desc(rentBills.billingPeriodStart));

  const tenantPayments = await dbClient
    .select()
    .from(payments)
